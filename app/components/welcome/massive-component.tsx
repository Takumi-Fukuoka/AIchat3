...
export const AppInfoComp: FC<{ siteInfo: AppInfo }> = ({ siteInfo }) => {
  ...
  return (
    <div>
      <div className='flex items-center py-2 text-xl font-medium text-gray-700 rounded-md'><img src={siteInfo.logoUrl} alt="Logo" className="flex items-center justify-center relative rounded-lg grow-0 shrink-0 overflow-hidden w-8 h-8 text-xl mr-2" />{siteInfo.title}</div>
...
export const ChatBtn: FC<{ onClick: () => void; className?: string }> = ({
  ...
  return (
    <Button
      type='primary'
      className={cn(className, `space-x-2 flex items-center ${s.customBtn}`) takStartBot}
      onClick={onClick}>
